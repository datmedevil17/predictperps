...
pub mod perp {
    ...
    pub fn create_market(
        ...
        handle_create_market(ctx, market_id)
    }

    /// Open a Long/Short position by staking collateral with leverage.
    pub fn open_position(ctx: Context<OpenPosition>, is_long: bool, collateral: u64, leverage: u64, current_price: u64) -> Result<()> {
        handle_open_position(ctx, is_long, collateral, leverage, current_price)
    }

    /// Close the position, calculating PnL based on current_price.
    ...
    pub fn delegate(ctx: Context<DelegatePositionPrivately>, members: Option<Vec<Member>>) -> Result<()> {
        handle_delegate(ctx, members)
    }

    /// Commit and undelegate the position back to the base layer.
    pub fn reveal_position(ctx: Context<RevealPosition>) -> Result<()> {
